...
## box indices 
def read_csv:
    boxes_GS=pd.read_csv('/home/albert7a/git/formation_ANNA/make_boxes/boxes_GS_1x1_eNATL60.csv',sep = '\t',index_col=0)
    imin_GS=boxes_GS['imin']
    imax_GS=boxes_GS['imax']
    jmin_GS=boxes_GS['jmin']
    jmax_GS=boxes_GS['jmax']
    box_name=boxes_GS.index
    return imin_GS,imax_GS,jmin_GS,jmax_GS,box_name
...
def compute_all_profiles(var,date,ibox):
    if var == 'buoyancy':
        ...
        data_boxT=dsT['votemper'][:,:,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
        filenameS = sorted(glob.glob(data_dir+'*/eNATL60-BLBT02_1h_*_gridS_'+date+'-'+date+'.nc'))
        fileS=filenameS[0]
        dsS=xr.open_dataset(fileS)
        data_boxS=dsS['vosaline'][:,:,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
        ...
        data_box=ds[str(var)][:,:,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
        attrs=data_box.attrs
        
    e1=dsgrid[str(filee1[var])][0,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
    e2=dsgrid[str(filee2[var])][0,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
    e3=dsgrid[str(filee3[var])][0,jmin_GS[ibox]-10:jmax_GS[ibox]+10,imin_GS[ibox]-10:imax_GS[ibox]+10]
...
def compute_all_profiles_all_var(date,ibox):
    ...
    profile_name='/scratch/cnt0024/hmg2840/albert7a/eNATL60/eNATL60-BLBT02-S/ANNA/GS/eNATL60GS'+box_name[ibox]+'-BLBT02_y'+date[0:4]+'m'+date[4:6]+'d'+date[6:9]+'_predictors-profiles.nc'
...
def main():
    ...
    imin_GS,imax_GS,jmin_GS,jmax_GS,box_name=read_csv()
    profile_name='/scratch/cnt0024/hmg2840/albert7a/eNATL60/eNATL60-BLBT02-S/ANNA/GS/eNATL60GS'+box_name[k]+'-BLBT02_y'+date[0:4]+'m'+date[4:6]+'d'+date[6:9]+'_predictors-profiles.nc'
